...
def compute_xs_from_y(img_left,img_right,img_left_test_y):
    ...
    roadDetector = EquirectMonoRoadDetector(roadSegmentator=roadSegmentator,window=window, degree=degree, debug=False)
    ...
    left_contour_points,right_contour_points=roadDetector._get_left_right_contours_of_one_image(img_left)


    left_poly_model_left = roadDetector.left_poly_model
    left_poly_model_right = roadDetector.right_poly_model

    left_contour_points,right_contour_points=roadDetector._get_left_right_contours_of_one_image(img_right)

    right_poly_model_left = roadDetector.left_poly_model
    right_poly_model_right = roadDetector.right_poly_model

    maxY, minY= roadDetector._get_road_top_and_bottom_v()
...
def find_matching(left_center, right_center, img_left, img_right,camTransform:Transform, template_size=30,debug=True, prefix=""):
    image_height, image_width = img_left.shape[:2]
    ...
    for meth in methods:
        ...
        if debug:
            ...
            cv2.circle(copyLeft, [left_image_x,left_image_y], 20, (0, 0, 255), 3)
            cv2.circle(copyRight, [right_image_matched_x,right_image_matched_y], 20, (0, 0, 255), 3)
            both = np.concatenate((copyLeft, copyRight), axis=0)
            cv2.imwrite(get_output_path( f"both{meth}_{prefix}.png"), both)
...
results_left = find_matching(
    ...
    img_left,img_right,cam_right_transform,template_size=50, prefix="left")

results_right = find_matching(
    [img_left_test_x_right,img_left_test_y],
    [img_right_test_x_right,img_right_test_y],
    img_left,img_right,cam_right_transform,template_size=50, prefix="right")
