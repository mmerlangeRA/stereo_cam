...
def getRefinedTransformFromKPMatching(
    ...
    initial_params = estimatedTransform.as_array()
...
def computeInliersIndices(
    ...
    if method_screen:
        ...
        u,v=cartesian_to_equirectangular(P2[:,0],P2[:,1],P2[:,2],image_width=image_width,image_height=image_height)
...
def auto_compute_cam2_transform(imLeft:cv2.Mat, imRight:cv2.Mat, estimatedTransform: Transform,transformBounds:TransformBounds,inlier_threshold:float,topLimit=0,bottomLimit =0, nn_match_ratio=0.5, verbose=False)->Tuple[Transform, float]:
    if verbose:
    ...
    kpts1, desc1 = detectAndComputeKPandDescriptors(imLeft,top_limit=topLimit,bottom_limit=bottomLimit)
    kpts2, desc2 = detectAndComputeKPandDescriptors(imRight,top_limit=topLimit,bottom_limit=bottomLimit)
    ...
    if refine_results:
        ...
        nb_inliers = len(inliersIndices)
        if verbose:
            print(f'refined best result with {nb_inliers} inliers vs {max_inliers}')
        if nb_inliers > max_inliers:
            best_transform = refine_transform
            max_inliers=nb_inliers
            if verbose:
    ...
    return best_transform,max_inliers/nb_good_matches
