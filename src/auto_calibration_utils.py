...
#akaze = cv2.AKAZE_create(threshold=1e-4,nOctaves=8, nOctaveLayers=8)
...
def match_features(images):
    keypoints_list, descriptors_list = zip(*[detect_and_compute_AKAZE(img) for img in images])
...
def compute_auto_calibration_for_images(images):
    ...
    #cv2.imwrite(f'img_matches_{index}.png', img_matches)
